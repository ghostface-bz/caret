...
function CaretMark() {
  return (
    <div
      className="flex h-[34px] w-[34px] items-center justify-center rounded-[9px] bg-lime text-accent-ink"
      style={{ boxShadow: "0 0 0 1px rgba(196,242,74,.4), 0 6px 16px rgba(196,242,74,.18)" }}
    >
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
        <path d="M7 8.5 L11 12 L7 15.5" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
        <rect x="12.5" y="14" width="6" height="2.2" rx="1.1" fill="currentColor" />
      </svg>
    </div>
  );
}

function ScansIcon({ className }: { className?: string }) {
  return (
...
function NewScanIcon({ className }: { className?: string }) {
  return (
    <svg className={className} width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `relative flex items-center gap-[11px] rounded-lg px-[11px] py-2 font-medium transition-colors ${
          isActive ? "bg-lime/10 text-lime-text" : "text-dim hover:bg-hover hover:text-ink"
        }`
      }
    >
      {({ isActive }) => (
    ...
    <div className="flex h-screen w-full overflow-hidden bg-base text-ink">
      ...
      <aside className="hidden w-[248px] flex-shrink-0 flex-col border-r border-line-side bg-bar md:flex">
        ...
        <nav className="flex flex-col gap-0.5 px-3 py-2">
          <NavItem to="/" end icon={<ScansIcon />} label="Scans" />
          <NavItem to="/new" icon={<NewScanIcon />} label="New scan" />
        </nav>
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-[57px] flex-shrink-0 items-center justify-between border-b border-line-side bg-base px-6">
          <div className="mono flex items-center gap-2 text-[13px] text-faint">
            <span className="text-lime-text md:hidden">caret</span>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <Link
              to="/new"
              className="flex items-center gap-1.5 rounded-lg bg-lime px-3.5 py-2 text-[13px] font-semibold text-accent-ink transition-colors hover:bg-lime-bright"
